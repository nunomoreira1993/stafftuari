...
if (!class_exists('WC_Lusopay_Integration')):
  ...
  // Recupera as configurações atuais salvas
   // 1. Forçar custom_field no $_POST para 'yes' antes do parent
    $_POST['woocommerce_multibanco-e-ou-payshop-by-lusopay_custom_field'] = 'yes';

    // 2. Chama o parent para salvar as outras opções
    parent::process_admin_options();

    // 3. Atualiza a propriedade da classe
    $this->custom_field = true;

    // 4. Atualiza custom_field na opção diretamente como 'yes'
    $settings = get_option('woocommerce_multibanco-e-ou-payshop-by-lusopay_settings', array());
//    $settings['custom_field'] = 'yes';
    update_option('woocommerce_multibanco-e-ou-payshop-by-lusopay_settings', $settings);

    // 5. Atualiza a data/hora do lusopaygateway_custom_field_2_value
  
    // 6. Processar outras opções enviadas pelo formulário
    if (isset($_POST['woocommerce_multibanco-e-ou-payshop-by-lusopay_chave'])) {
        $chave = sanitize_text_field($_POST['woocommerce_multibanco-e-ou-payshop-by-lusopay_chave']);
        $settings['chave'] = $chave;
        update_option('woocommerce_multibanco-e-ou-payshop-by-lusopay_settings', $settings);
    }
    ...
    // Atualiza a propriedade da classe para refletir imediatamente
   // $this->custom_field = 'yes';

    // Atualiza também a data/hora do custom_field_2_value
        ...
        update_option('lusopaygateway_custom_field_2_value', $dt->format('d-m-Y, H:i:s'));
			...
			parent::process_admin_options();
		if ($this->settings['custom_field'] === 'no') {
    $tz = 'Europe/London';
    $timestamp = time();
   ...
   // update_option('lusopaygateway_custom_field_2_value', false);
}
		 }
