...
$array[] = array("Codigo", "Área", "Mesa", "Data / Hora de venda", "Nome processado", "Nome do cliente", "Gerente", "Staff", "Valor Multibanco Adiantado", "Valor Dinheiro Adiantado", "Valor MBWAY Adiantado", "Valor Transferência Bancária Adiantado", "Total Adiantado", "Valor Multibanco", "Valor Dinheiro", "Valor MBWAY", "Total Pago Evento", "Total Camarote");
foreach ($vendas as $venda) {
    $array[] = array($venda['codigo'], $venda['sala'], $venda['mesa'], $venda['data'], $venda['nome_processado'], $venda['nome_cliente'], $venda['nome_gerente'], $venda['nome_rp'], number_format($venda['valor_multibanco_adiantado'], 2, ',', '.') . " €", number_format($venda['valor_dinheiro_adiantado'], 2, ',', '.') . " €", number_format($venda['valor_mbway_adiantado'], 2, ',', '.') . " €", number_format($venda['valor_transferencia_bancaria_adiantado'], 2, ',', '.') . " €", number_format($venda['valor_multibanco_adiantado'] + $venda['valor_dinheiro_adiantado'] + $venda['valor_mbway_adiantado'] + $venda['valor_transferencia_bancaria_adiantado'], 2, ',', '.') . " €", number_format($venda['valor_multibanco'], 2, ',', '.') . " €", number_format($venda['valor_dinheiro'], 2, ',', '.') . " €", number_format($venda['valor_mbway'], 2, ',', '.') . " €", number_format($venda['valor_multibanco'] + $venda['valor_dinheiro'] + $venda['valor_mbway'], 2, ',', '.') . " €", number_format($venda['total'], 2, ',', '.') . " €");
}
